...
class Tetromino {
  ...
  drawGhost(canvas, board, width = BLOCK_SIZE, height = BLOCK_SIZE) {
    ...
    for (var i = 0; i < 4; i++) {
      drawMonomino(
        "g",
        this.xs[i] * width,
        (this.ys[i] + yOffset - HIDDEN_ROWS) * height,
        canvas,
        width,
        height
      );
  ...
  drawFixed(canvas, x = 0, y = 0, width = BLOCK_SIZE, height = BLOCK_SIZE) {
    ...
    for (var i = 0; i < 4; i++) {
      drawMonomino(
        this.letter,
        (xOffset + this.xs[i]) * width,
        (yOffset + this.ys[i]) * height,
        canvas,
        width,
        height
      );
...
class Game {
  ...
  update() {
    ...
    if (this.clearTimer >= 0) {
      ...
      if (this.clearTimer == 0) {
        ...
        var sortedLines = [...this.linesCleared].sort(function (a, b) {
          return a - b;
        });
        ...
        if (this.trainingMode) {
          ...
          if (highestColumn > 10 + this.level) {
            for (
              var row = highestColumn - trainingHeight;
              row < HEIGHT - trainingHeight;
              row++
            ) {
              for (var col = 0; col < WIDTH; col++) {
  ...
  processReplayInput() {
    ...
    //Move left or right
    if (
      userInput.indexOf("left") >= 0 &&
      !this.current.collision(this.board, -1, 0)
    ) {
      this.current.translate(-1, 0);
      inputAccepted = true;
    } else if (
      userInput.indexOf("right") >= 0 &&
      !this.current.collision(this.board, 1, 0)
    ) {
      this.current.translate(1, 0);
      inputAccepted = true;
  ...
  processUserInput() {
    ...
    var firstTick =
      this.moveTimer == this.currentPeriod || this.moveTimer == this.setPeriod;
    var thisInput = "";
  ...
  draw() {
    ...
    if (this.alive && this.active) {
      ...
      this.current.draw(
        this.canvas,
        this.setTimer % 4 < 2 && this.setTimer > 0
      );
      ...
      context.fillStyle = WHITE;
      context.fillRect(
        (WIDTH / 2 - 2) * BLOCK_SIZE,
        (1 / 3) * HEIGHT * BLOCK_SIZE,
        BLOCK_SIZE,
        ((1 / 3) * HEIGHT - 1) * BLOCK_SIZE
      );
      context.fillRect(
        (WIDTH / 2 + 1) * BLOCK_SIZE,
        (1 / 3) * HEIGHT * BLOCK_SIZE,
        BLOCK_SIZE,
        ((1 / 3) * HEIGHT - 1) * BLOCK_SIZE
      );
